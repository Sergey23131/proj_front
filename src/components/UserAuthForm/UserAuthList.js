import {useEffect, useState} from "react";
import {deleteUser, getUserByID, logoutUser} from "../../services/service";
import './userAuthCard.css'
import {Link, Route} from "react-router-dom";
import {useHistory} from "react-router";
import {UpdateForm} from "../UpdateForm/UpdateForm";


export function UserAuth() {
    const [hidden, setHidden] = useState(false);
    const [user, setUser] = useState({});
    const history = useHistory();
    const user_id = localStorage.user_id;


    useEffect(() => {
        getUserByID(user_id).then(value => setUser(value))
    }, [user]);

    const updateState = () => {
        setHidden(false)
    }

    const updateButton = () => {
        history.push('/auth/update')

        setHidden(true)
    }

    const logoutButton = async () => {
        await logoutUser(localStorage.access_token)

        history.push('/MainList')
    }

    const deleteButton = async () => {
        await deleteUser(user_id).then(value => console.log(value))

    }

    return (
        <div className="UserAuth">
            <div className={'headerAuthCard'}>
                <Link to={'/MainList'} className={'BackButton'}>Back</Link>
                <button onClick={logoutButton} className={'LogoutButton'}>Log out</button>
            </div>

            <div className={'userCard'}>
                <div className={'user-Info'}>
                    <h2>User Card</h2>
                    <p><b>Name:</b> {user.name}</p>
                    <p><b>Nick name:</b> {user.nickName}</p>
                    <p><b>Age:</b> {user.age}</p>
                    <p><b>Role:</b> {user.role}</p>
                    <p><b>Email:</b> {user.email}</p>
                    <p><b>Phone:</b> {user.phoneNumber}</p>

                    <p><b>Info:</b> Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusamus aliquid
                        aspernatur assumenda
                        at atque consectetur consequatur consequuntur cum debitis ducimus ea eius exercitationem facere
                        fuga hic illum inventore ipsam itaque iure labore, minus necessitatibus nemo nisi odit placeat
                        praesentium quam quibusdam quidem ratione repellat repellendus reprehenderit similique sit
                        tempora unde velit voluptates. Accusamus consequuntur, debitis dolore dolorum est exercitationem
                        expedita, facere nobis, odio praesentium sit voluptatem. Aspernatur assumenda harum ipsa minus
                        nam nisi officia perspiciatis quis. Beatae incidunt ipsam numquam quas soluta. Aut eum incidunt
                        iste minima officia quaerat quo saepe sunt! Ab architecto eius eveniet fugiat natus rerum
                        vel.</p>


                </div>

                <div className={'user-screen'}>

                    <img
                        src="data:image/png;base64,"
                        alt="userPhoto"/>

                    {!hidden && <button onClick={updateButton} className={'UpdateButton'}>Update</button>}


                    <Route path={"/auth/update"} render={(props) => {
                        return <UpdateForm {...props} updateState={updateState}/>
                    }}/> <br/>

                    <button onClick={deleteButton} className={'deleteButton'}>Delete user</button>
                </div>


            </div>

        </div>
    );
}